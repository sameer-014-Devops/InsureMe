...
Variable: 
  ...
  KubeW01_Pub_IP =
  KubeW02_Pub_IP =
  Prod_Workspace = /home/${User_Name}/workspace/InsureMe_Production_Pipeline
  Build01_Pvt_IP =
...
pipeline {
    ...
    stages {
        ...
        stage('SCM_Checkout') {
            ...
            steps {
                ...
                git 'https://github.com/sameer-014-Devops/InsureMe.git'
            }
            post {
                success {
                    script {
        ...
        stage('Verify_Deploy_Test') {
            ...
            steps {
                ...
                sleep 15
                script {
                    def response = sh(script: """curl -s -o /dev/null -w '%{http_code}'  http://$Test_Server_Pub_IP:8083""", returnStdout: true).trim()
                    if (response == '200') {
                        Deploy_Verify = true
        ...
        stage('Verification_User'){
            ...
            steps{
                ...
                script{
                    echo "The Application URL: http://$Test_Server_Pub_IP:8083"
        ...
        stage ('Copy_Files_To_Production_Workspace'){
            ...
            steps{
                ...
                echo '**********Copied the Dockerfile, insuremedeploy.yaml, and target files to the InsureMe Production workspace**********'
                cleanWs()
                deleteDir()
            }
        }
    ...
    post {
        success {
            build job: 'InsureMe_Production_Pipeline', parameters: [string(name: 'new_ver', value: env.new_ver)]
